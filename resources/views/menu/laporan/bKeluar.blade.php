    ...
    <div class="p-6 space-y-4">
        ...
        <div class="flex items-end gap-4 border rounded-lg p-4 bg-white flex-wrap">
            ...
            <form action="{{ route('streamPDF.bKeluar.view') }}" method="post" class="flex flex-col justify-end">
             ...
             <div class="flex flex-col justify-end">
                <a href="{{route('laporan.bKeluar.view')}}" class="px-4 py-1.5 text-sm font-medium text-white bg-pink-500 rounded-md hover:bg-pink-600 h-[42px] text-center justify-center items-center"> Reset Filter </a>
            </div>
                    ...
                    @foreach ($bKeluar as $data)
                        @foreach ($data->detailKeluar as $detail)
                            <tr>
                                ...
                                <td class="px-4 py-2  text-left">{{ $detail->barangDetailKeluar->barang->namaBarang }}</td>
